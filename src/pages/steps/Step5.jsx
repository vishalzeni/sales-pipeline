...
const Step5 = ({ stepName, cards, setStepsData }) => {
    ...
    drop: (item) => {
      ...
      if (fromStep !== stepName) {
        setStepsData((prevStepsData) => {
          ...
          // Update stepsData in local storage
  ...
  // Effect to sync Step5 cards to "customer" key in localStorage
  useEffect(() => {
    const savedStepsData = JSON.parse(localStorage.getItem('stepsData'));
    if (savedStepsData?.Step5) {
      // Store Step5 data in "customer"
      localStorage.setItem('customer', JSON.stringify(savedStepsData.Step5));
    }
  }, [cards]); // Runs whenever Step5 cards are updated
